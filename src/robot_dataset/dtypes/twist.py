...
class TwistConvert(Dtype):
    ...
    def obs_space(self):
        if self.mode=="state":
            return Box(low=-1, high=1, shape=(6,), dtype=np.float32)  # TODO: Check values for limits
        else:
            return None
        
    def action_space(self):
        if self.mode=="action":
            return Box(low=-1, high=1, shape=(2,), dtype=np.float32) # TODO: Check values for limits
        else: 
            return None
