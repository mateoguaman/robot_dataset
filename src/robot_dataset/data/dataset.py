...
from robot_dataset.types import DataType

DatasetDict = Dict[str, DataType]


# Unsure if needed, but seems to be a way to check if all the elements in the dataset dict have the same length
...
def _sample(dataset_dict: Union[np.ndarray, DatasetDict], indx: np.ndarray) -> DatasetDict:
    if isinstance(dataset_dict, np.ndarray):
        ...
        raise TypeError("Unsopported type.")
    return batch
